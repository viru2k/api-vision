...
class FacturaElementosController extends ApiController
{
  ...
  {
    $factura = DB::select(
      DB::raw("SELECT agenda_dia_horario_atencion.id, agenda_usuario_dia_horario_id, agenda_dia_horario_atencion.usuario_alta_id, fecha_turno, presente, llegada, atendido, agenda_estado_id, paciente_id, observacion, es_alerta, es_observacion, operacion_cobro_id, es_sobreturno, agenda_dia_horario_atencion.created_at, agenda_dia_horario_atencion.updated_at, CONCAT(paciente.apellido, ' ',paciente.nombre) AS paciente_nombre  , users.nombreyapellido, users.id as usuario_id
  ...
  public function crearFactura(Request $request)
  {
    ...
    if ($request->factura_comprobante_id != 11) {
      ...
      foreach ($request->facturaElectronicaRenglon as $s) {
        if (
          $request->facturaElectronicaRenglon[$j]["tipo_concepto_iva"][
            "name"
          ] === "GRAVADO"
        ) {
          if ($s["alicuota_id"] == 5) {
    ...
    if ($resAfip) {
      $factura_encabezado_id = DB::table("factura_encabezado")->insertGetId([
        ...
        "importe_exento_iva" =>
          $request->importe_total_neto_no_gravado + $request->importe_op_exenta,
        "importe_iva" => $request->importe_iva,
        "importe_total" => $request->importe_total,
      ]);
