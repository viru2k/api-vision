...
namespace App\Http\Controllers\V2\Afip;
use Afip;
...
class AfipController extends Controller
{
  ...
  public function testAfipGetLastVoucher()
  {
    $medico = DB::select(
      DB::raw(
        "SELECT cuit, factura_key, factura_crt FROM medicos WHERE id = 24"
      )
    );
    $afip = new Afip([
      "CUIT" => 30681711445,
  ...
  public function GetVoucherInfo(Request $request)
  {
    ...
    //Esta línea es solo para probar si funciona pero no es obligatorio
    $voucher_info = $afip->ElectronicBilling->GetVoucherInfo(
      $request->input("comprobante_numero"),
      $request->input("punto_vta"),
      $request->input("comprobante_id")
  ...
  public function CrearFacturaA(Request $request)
  {
    ...
    $data = [
      ...
      "Iva" => [
        // Alícuotas asociadas al factura
        [
  ...
  public function CrearFacturaC(Request $request)
  {
    $medico = DB::select(
      DB::raw(
        "SELECT cuit, factura_key, factura_crt, factura_punto_vta_medico.punto_vta FROM medicos, factura_punto_vta_medico WHERE medicos.id = factura_punto_vta_medico.medico_id AND medicos.id = " .
          $request->input("id") .
          ""
      )
    );
  ...
  public function CrearNotaCreditoA(Request $request)
  {
    ...
    $data = [
      ...
      "Iva" => [
        // Alícuotas asociadas a la Nota de Crédito
        [
          "Id" => 5, // Id del tipo de IVA (5 = 21%)
          "BaseImp" => $importe_gravado,
          "Importe" => $importe_iva,
  ...
  {
    $medico = DB::select(
      DB::raw("SELECT medicos.id,CONCAT(apellido,' ',nombre) AS nombreyapellido , domicilio, fecha_matricula, cuit, ing_brutos, usuario_id, factura_key, factura_crt, 
  ...
  public function getDatoMedico(Request $request)
  {
    ...
    $medico = DB::select(
      DB::raw(
        "SELECT medicos.id,CONCAT(apellido,' ',nombre) AS nombreyapellido , domicilio, fecha_matricula, cuit, ing_brutos, usuario_id, factura_key, factura_crt, 
                categoria_iva.categoria_iva, factura_documento_comprador.id  AS factura_documento_comprador_id, factura_documento_comprador.descripcion, factura_punto_vta.id AS  factura_punto_vta_id, 
                ...
                FROM medicos, categoria_iva, factura_documento_comprador, factura_punto_vta, factura_comprobante WHERE medicos.punto_vta_id = factura_punto_vta.id AND factura_documento_comprador.id = medicos.factura_documento_comprador_id AND  medicos.factura_comprobante_id = factura_comprobante.id AND   cuit != '' AND factura_key != '' AND factura_crt !='' AND medicos.categoria_iva_id = categoria_iva.id AND medicos.id = " .
          $medico_id .
